...
public class TelegramFileContent
{
 ...
 public  async Task<List<MessageSentResult>?> Send(TelegramBotAbstract sender, int loopNumber, string? langCode, long? replyToMessageId2, MessageEventArgs? messageEventArgs)
 {
  ...
  if (string.IsNullOrEmpty(this.FileContent))
  {
   ...
   var r11 = await Utils.NotifyUtil.NotifyOwners7(text1, sender, loopNumber, langCode, replyToMessageId2, messageEventArgs, this.FileContent);
   return r11;
  }
  ...
  var r1 = await Utils.NotifyUtil.NotifyOwners7(text, sender, loopNumber, langCode, replyToMessageId2, messageEventArgs);
  return r1;
 }
}