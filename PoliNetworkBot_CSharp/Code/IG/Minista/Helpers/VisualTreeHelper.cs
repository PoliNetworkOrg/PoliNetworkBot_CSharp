    ...
    public static class VisualTreeHelperExtensions
    {
        ...
        public static UIElement GetRealWindowRoot(Window window = null)
        {
            ...
            if (root != null)
            {
                ...
                if (ancestors.Count > 0) root = (FrameworkElement)ancestors[ancestors.Count - 1];
            }
        ...
        /// <param name="origin">
        ///     The relative (0..1,0..1 range) position of a point within the element to evaluate. Defaults to 0,0
        ///     for top-left corner.
        /// </param>
        ...
        public static Point GetPosition(this UIElement dob, Point origin = new(), UIElement relativeTo = null)
        {
            if (DesignMode.DesignModeEnabled) return new Point();

            if (relativeTo == null) relativeTo = Window.Current.Content;

            if (relativeTo == null) throw new InvalidOperationException("Element not in visual tree.");
            ...
            if (dob == relativeTo) return absoluteOrigin;

            var ancestors = dob.GetAncestors().ToArray();

            if (!ancestors.Contains(relativeTo)) throw new InvalidOperationException("Element not in visual tree.");
        ...
        {
            if (DesignMode.DesignModeEnabled) return Rect.Empty;

            if (relativeTo == null) relativeTo = Window.Current.Content as FrameworkElement;
