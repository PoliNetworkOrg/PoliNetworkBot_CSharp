...
namespace InstagramApiSharp.API.Processors
{
    ...
    internal class LiveProcessor : ILiveProcessor
    {
        ...
        public async Task<IResult<InstaBroadcastCommentEnableDisable>> DisableCommentsAsync(string broadcastId)
        {
            ...
            {
                _logger?.LogException(httpException);
                return Result.Fail(httpException, default(InstaBroadcastCommentEnableDisable),
        ...
        public async Task<IResult<InstaBroadcastCommentList>> GetCommentsAsync(string broadcastId,
            string lastCommentTs = "", int commentsRequested = 4)
        {
            UserAuthValidator.Validate(_userAuthValidate);
            try
            {
            ...
            string broadcastId)
        {
            ...
            {
                _logger?.LogException(httpException);
                return Result.Fail(httpException, default(InstaBroadcastLiveHeartBeatViewerCount),
                    ResponseType.NetworkProblem);
            }
            catch (Exception exception)
            {
        ...
        public async Task<IResult<InstaBroadcastTopLiveStatusList>> GetTopLiveStatusAsync(params string[] broadcastIds)
        {
            ...
            {
                _logger?.LogException(httpException);
                return Result.Fail(httpException, default(InstaBroadcastTopLiveStatusList),
                    ResponseType.NetworkProblem);
            }
            catch (Exception exception)
            {
        ...
        public async Task<IResult<object>> SeenBroadcastAsync(string broadcastId, string pk)
        {
            ...
            {
                _logger?.LogException(httpException);
                return Result.Fail(httpException, default(InstaBroadcastLiveHeartBeatViewerCountResponse),
                    ResponseType.NetworkProblem);
            }
            catch (Exception exception)
            {
