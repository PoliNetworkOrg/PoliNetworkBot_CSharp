...
namespace InstagramApiSharp.API.Processors
{
    ...
    internal class CommentProcessor : ICommentProcessor
    {
        ...
        /// <returns></returns>
        public async Task<IResult<InstaInlineCommentList>> GetMediaRepliesCommentsAsync(string mediaId,
            ...
            PaginationParameters paginationParameters)
        {
            ...
            try
            {
                ...
                var commentsUri =
                    UriCreator.GetMediaInlineCommentsUri(mediaId, targetCommentId, paginationParameters.NextMaxId);
                if (!string.IsNullOrEmpty(paginationParameters.NextMinId))
                    commentsUri = UriCreator.GetMediaInlineCommentsWithMinIdUri(mediaId, targetCommentId,
                       ...
                       && pagesLoaded < paginationParameters.MaximumPagesToLoad)
                {
                    ...
                    if (!string.IsNullOrEmpty(commentListResponse.NextMaxId))
                        nextComments = await GetInlineCommentListWithMaxIdAsync(mediaId, targetCommentId,
                            commentListResponse.NextMaxId, null);
                    else
                        nextComments = await GetInlineCommentListWithMaxIdAsync(mediaId, targetCommentId, null,
