...
internal class LocationProcessor : ILocationProcessor
{
    ...
    public async Task<IResult<InstaPlaceShort>> GetLocationInfoAsync(string externalIdOrFacebookPlacesId)
    {
        try
        {
            ...
            return Result.Success(ConvertersFabric.GetPlaceShortConverter(obj.Location).Convert());
        }
        catch (HttpRequestException httpException)
        {
            _logger?.LogException(httpException);
    ...
    public async Task<IResult<InstaSectionMedia>> GetRecentLocationFeedsAsync(long locationId,
    ...
    /// <param name="locationId">Location identifier (location pk, external id, facebook id)</param>
    /// <param name="paginationParameters">Pagination parameters: next id and max amount of pages to load</param>
    public async Task<IResult<InstaSectionMedia>> GetTopLocationFeedsAsync(long locationId,
        PaginationParameters paginationParameters)
    {
        ...
        try
        {
                   ...
                   && paginationParameters.PagesLoaded < paginationParameters.MaximumPagesToLoad)
            {
                ...
                if (!moreMedias.Succeeded)
                    return mediaResponse.Value.Sections?.Count > 0
                        ? Result.Success(Convert(mediaResponse.Value))
                        : Result.Fail(moreMedias.Info, Convert(mediaResponse.Value));
