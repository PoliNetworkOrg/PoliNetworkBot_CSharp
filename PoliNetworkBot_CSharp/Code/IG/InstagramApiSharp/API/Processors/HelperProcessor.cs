...
internal class HelperProcessor
{
        ...
        InstaVideoUpload video, Uri uri = null, InstaStoryUploadOptions uploadOptions = null)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                request = _httpHelper.GetSignedRequest(UriCreator.GetStoryMediaInfoUploadUri(),
                    _deviceInfo, videoUploadParamsObj);
                response = await _httpRequestProcessor.SendAsync(request);
                json = await response.Content.ReadAsStringAsync();
            ...
            if (isDirectVideo)
                return await ConfigureVideo(progress, upProgress, uploadId, isDirectVideo, isDisappearingVideo,
                    caption,
                    viewMode, storyType, recipients, threadId, uri, uploadOptions);
            ...
            return await ConfigureVideo(progress, upProgress, uploadId, isDirectVideo, isDisappearingVideo, caption,
                viewMode, storyType, recipients, threadId, uri, uploadOptions);
        }
        ...
        string recipients, string threadId, InstaImage image)
    {
        ...
        try
        {
            ...
            return await ConfigurePhoto(progress, upProgress, uploadId, isDirectPhoto, isDisappearingPhoto, caption,
                viewMode, storyType, recipients, threadId);
        }
        catch (HttpRequestException httpException)
        {
            _logger?.LogException(httpException);
        ...
        string caption, InstaViewMode viewMode, InstaStoryType storyType, string recipients, string threadId)
    {
        try
        {
            ...
            //else
            {
                ...
                instaUri = UriCreator.GetVideoStoryConfigureUri();
                var request = _httpHelper.GetSignedRequest(instaUri, _deviceInfo, data);
        ...
        List<InstaUserTagUpload> userTags = null)
    {
        try
        {
            ...
            var rootJsonObj = JObject.Parse(json);
    ...
    public async Task<IResult<InstaMedia>> SendIGTVVideoAsync(Action<InstaUploaderProgress> progress,
        InstaVideoUpload video, string title, string caption)
    {
        var upProgress = new InstaUploaderProgress
        {
            Caption = caption ?? string.Empty,
