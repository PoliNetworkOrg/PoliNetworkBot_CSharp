...
#pragma warning restore IDE0052 // Remove unread private members
    ...
    public async Task<IResult<InstaSignupConsentConfig>> GetSignupConsentConfigAsync(bool isMainAccount = false,
        long? loggedInUserId = null)
    {
        try
        {
            var instaUri = UriCreator.GetSignupConsentConfigUri(_deviceInfo.DeviceGuid.ToString(), isMainAccount,
                loggedInUserId);
            ...
            return obj.IsSucceed
                ? Result.Success(obj)
                : Result.UnExpectedResponse<InstaSignupConsentConfig>(response, json);
        }
        catch (HttpRequestException httpException)
        {
            _logger?.LogException(httpException);
        ...
        string email, string verificationCode)
    {
        try
        {
            ...
            return obj.IsSucceed
                ? Result.Success(obj)
                : Result.UnExpectedResponse<InstaRegistrationConfirmationCode>(response, json);
        }
        catch (HttpRequestException httpException)
        {
            _logger?.LogException(httpException);
            return Result.Fail(httpException, default(InstaRegistrationConfirmationCode),
    ...
    public async Task<IResult<bool>> GetSiFetchHeadersAsync()
    {
        return await GetResultAsync(
            UriCreator.GetSiFetchHeadersUri(_deviceInfo.DeviceGuid.ToString().Replace("-", "")),
    ...
    /// </summary>
    /// <param name="birthday">
    ///     Birthday => Passing null, will generate randomly and save it to
    ///     <see cref="IRegistrationService.Birthday" />
    /// </param>
    public async Task<IResult<InstaCheckAgeEligibility>> CheckAgeEligibilityAsync(DateTime? birthday = null)
    {
        try
        {
            ...
            return obj.IsSucceed
                ? Result.Success(obj)
                : Result.UnExpectedResponse<InstaCheckAgeEligibility>(response, json);
        }
        catch (HttpRequestException httpException)
        {
            _logger?.LogException(httpException);
    ...
    public async Task<IResult<bool>> GetOnboardingStepsAsync(InstaOnboardingProgressState progressState,
        InstaRegistrationMethod registrationMethod = InstaRegistrationMethod.Email)
    {
        try
        {
            RegistrationWaterfallId = Guid.NewGuid().ToString();
            ...
            {
                {
                    ...
                    progressState == InstaOnboardingProgressState.Prefetch ? "false" : "true"
                },
                { "fb_connected", "false" },
            ...
            if (registrationMethod == InstaRegistrationMethod.Email)
            {
                switch (progressState)
                {
                    ...
                    case InstaOnboardingProgressState.Finish:
                        data.Add("seen_steps",
                            "[{\"step_name\":\"CHECK_FOR_PHONE\",\"value\":1},{\"step_name\":\"FB_CONNECT\",\"value\":0},{\"step_name\":\"FB_FOLLOW\",\"value\":-1},{\"step_name\":\"UNKNOWN\",\"value\":-1},{\"step_name\":\"CONTACT_INVITE\",\"value\":-1},{\"step_name\":\"ACCOUNT_PRIVACY\",\"value\":-1},{\"step_name\":\"TAKE_PROFILE_PHOTO\",\"value\":0},{\"step_name\":\"ADD_PHONE\",\"value\":-1},{\"step_name\":\"TURN_ON_ONETAP\",\"value\":-1}]");
                        break;
                }
            }
            else
    ...
    {
        return await GetResultAsync(UriCreator.GetMultipleAccountsFamilyUri(true)).ConfigureAwait(false);
    }
    ...
    {
        return await GetResultAsync(UriCreator.GetZrTokenResultUri(_deviceInfo.DeviceGuid.ToString(),
            _deviceInfo.DeviceId, true)).ConfigureAwait(false);
    }
