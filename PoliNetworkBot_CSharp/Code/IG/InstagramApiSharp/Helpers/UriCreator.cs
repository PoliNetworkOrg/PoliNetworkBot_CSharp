...
namespace InstagramApiSharp.Helpers
{
    ...
    {
        private static readonly Uri BaseInstagramUri = new(InstaApiConstants.INSTAGRAM_URL);
        private static readonly Uri BaseInstagramBUri = new(InstaApiConstants.INSTAGRAM_B_URL);

        public static Uri GetAcceptFriendshipUri(long userId)
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.FRIENDSHIPS_APPROVE, userId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_ADD_TO_POST_LIVE, broadcastId),
                    out var instaUri))
        ...
        public static Uri GetBroadcastCommentUri(string broadcastId, string lastcommentts = "")
        {
            if (lastcommentts == "")
            {
                if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_GET_COMMENT, broadcastId),
                        out var instaUri))
            ...
            else
            {
                if (!Uri.TryCreate(BaseInstagramUri,
                        string.Format(InstaApiConstants.LIVE_GET_COMMENT_LASTCOMMENTTS, broadcastId, lastcommentts),
                        out var instaUri))
                    throw new Exception("Cant create URI for broadcast get comments");
                return instaUri;
            }
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_DELETE_POST_LIVE, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_MUTE_COMMENTS, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_UNMUTE_COMMENTS, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_END, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_INFO, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_GET_JOIN_REQUESTS, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_PIN_COMMENT, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_COMMENT, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    string.Format(InstaApiConstants.LIVE_GET_POST_LIVE_COMMENT, broadcastId, startingOffset,
                        encodingTag), out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    string.Format(InstaApiConstants.LIVE_POST_LIVE_LIKES, broadcastId, startingOffset, encodingTag),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_START, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_UNPIN_COMMENT, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_GET_VIEWER_LIST, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    InstaApiConstants.API_SUFFIX + apiPath.Replace("challenge/", "challenge/replay/"),
                    out var instaUri))
                throw new Exception("Cant create URI for challenge require url");
            return instaUri;
        }
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.FRIENDSHIPS_IGNORE, userId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, InstaApiConstants.DIRECT_BROADCAST_LIVE_VIEWER_INVITE,
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.DIRECT_THREAD_UPDATE_TITLE, threadId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.HIGHLIGHT_EDIT_REEL, highlightId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.HIGHLIGHT_TRAY, userId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_LIKE, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    string.Format(InstaApiConstants.LIVE_GET_FINAL_VIEWER_LIST, broadcastId), out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    string.Format(InstaApiConstants.LIVE_HEARTBEAT_AND_GET_VIEWER_COUNT, broadcastId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.LIVE_GET_LIKE_COUNT, broadcastId),
                    out var instaUri))
        ...
        public static Uri GetMediaConfigureUri(bool video = false)
        {
            if (
                !Uri.TryCreate(BaseInstagramUri,
                    video ? InstaApiConstants.MEDIA_CONFIGURE_VIDEO : InstaApiConstants.MEDIA_CONFIGURE,
                    out var instaUri))
        ...
        public static Uri GetMediaInlineCommentsWithMinIdUri(string mediaId, string targetCommentId,
            string nextMinId = "")
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    string.Format(InstaApiConstants.MEDIA_INLINE_COMMENTS, mediaId, targetCommentId),
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.DIRECT_THREAD_MUTE, threadId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(isBUrl ? BaseInstagramBUri : BaseInstagramUri,
                    InstaApiConstants.DYNAMIC_ONBOARDING_GET_STEPS, out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    string.Format(InstaApiConstants.GET_PARTICIPANTS_RECIPIENT_USER, userId), out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    string.Format(InstaApiConstants.GET_RANK_RECIPIENTS_BY_USERNAME, username), out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, InstaApiConstants.ACCOUNTS_SEND_TWO_FACTOR_ENABLE_SMS,
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.STORY_SHARE, mediaType),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    string.Format(InstaApiConstants.FBSEARCH_SUGGESTED_SEARCHS, searchType.ToString().ToLower()),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    string.Format(InstaApiConstants.FBSEARCH_TOPSEARCH_FALT_PARAMETER, rankToken, timezone_offset,
                        querry, searchType.ToString().ToLower()), out var instaUri))
                throw new Exception("Cant create URI for suggested search");
            return instaUri;
        }
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.DIRECT_THREAD_UNMUTE, threadId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    isVideo ? InstaApiConstants.STORY_CONFIGURE_VIDEO : InstaApiConstants.STORY_CONFIGURE_VIDEO2,
                    out var instaUri))
                throw new Exception("Can't create URI for configuring story media");
            return instaUri;
        }
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.DIRECT_THREAD_ADD_USER, threadId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.FRIENDSHIPS_MARK_USER_OVERAGE, userId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.FRIENDSHIPS_FAVORITE, userId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.FRIENDSHIPS_UNFAVORITE, userId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    string.Format(InstaApiConstants.FRIENDSHIPS_FAVORITE_FOR_STORIES, userId), out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri,
                    string.Format(InstaApiConstants.FRIENDSHIPS_UNFAVORITE_FOR_STORIES, userId), out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, InstaApiConstants.BUSINESS_CONVERT_TO_BUSINESS_ACCOUNT,
                    out var instaUri))
        ...
        public static Uri GetPresenceUri(string signedKey)
        {
            ...
            var query =
                $"{InstaApiConstants.HEADER_IG_SIGNATURE}={signedBody}&{InstaApiConstants.HEADER_IG_SIGNATURE_KEY_VERSION}={InstaApiConstants.IG_SIGNATURE_KEY_VERSION}";
            var uriBuilder = new UriBuilder(instaUri) { Query = query };
            return uriBuilder.Uri;
        }
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.MEDIA_FOLLOW_COUNTDOWN, countdownId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.MEDIA_UNFOLLOW_COUNTDOWN, countdownId),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(BaseInstagramUri, string.Format(InstaApiConstants.TAG_SECTION, hashtag),
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(isBUrl ? BaseInstagramBUri : BaseInstagramUri, InstaApiConstants.LAUNCHER_SYNC,
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(isBUrl ? BaseInstagramBUri : BaseInstagramUri,
                    InstaApiConstants.ACCOUNTS_CONTACT_POINT_PREFILL, out var instaUri))
        ...
        {
            if (!Uri.TryCreate(isBUrl ? BaseInstagramBUri : BaseInstagramUri, InstaApiConstants.QE_SYNC,
                    out var instaUri))
        ...
        {
            if (!Uri.TryCreate(isBUrl ? BaseInstagramBUri : BaseInstagramUri,
                    InstaApiConstants.MULTIPLE_ACCOUNTS_GET_ACCOUNT_FAMILY, out var instaUri))
        ...
        {
            if (!Uri.TryCreate(isBUrl ? BaseInstagramBUri : BaseInstagramUri,
                    InstaApiConstants.NUX_NEW_ACCOUNT_NUX_SEEN, out var instaUri))
