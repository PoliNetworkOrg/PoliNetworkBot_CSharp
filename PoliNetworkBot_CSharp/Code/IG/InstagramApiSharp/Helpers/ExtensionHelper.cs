...
namespace InstagramApiSharp
{
    internal static class ExtensionHelper
    {
        ...
        public static string GetEncryptedPassword(this InstaApi api, string password, long? providedTime = null)
        {
            ...
            encKeyIdBytes[0] = 1;
            var payload = Convert.ToBase64String(encKeyIdBytes.Concat(iv).Concat(buffersSize).Concat(encryptedKey)
        ...
        public static string GenerateSnNonce(string emailOrPhoneNumber)
        {
            var b = new byte[24];
            Rnd.NextBytes(b);
            var str = $"{emailOrPhoneNumber}|{DateTime.UtcNow.ToUnixTime()}|{Encoding.UTF8.GetString(b)}";
