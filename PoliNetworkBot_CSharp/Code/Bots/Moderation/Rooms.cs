...
internal static class Rooms
{
    ...
    /// </summary>
    private static async Task<Tuple<ExceptionNumbered,List<HtmlNode>>> GetDailySituationAsync(TelegramBotAbstract sender, MessageEventArgs e)
    {
        var (dateTimeSchedule, exception, item3) = await AskUser.AskDateAsync(e.Message.From?.Id,
            "Scegli un giorno", "it", sender,
            e.Message.From?.Username);

        if (exception != null) return new Tuple<ExceptionNumbered, List<HtmlNode>>(new ExceptionNumbered(exception), null);
