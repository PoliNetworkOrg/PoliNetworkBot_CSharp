...
namespace PoliNetworkBot_CSharp.Code.Utils
{
    internal class DateTimeClass
    {
        ...
        private static async System.Threading.Tasks.Task<DateTime?> AskDate2Async(int id, string lang, TelegramBotAbstract sender)
        {
            ...
            DateTime? reply_Datetime = DateTimeClass.GetDateTimeFromString(reply);
        ...
        private static DateTime? GetDateTimeFromString2(string reply)
        {
            ...
            else if (reply.StartsWith("un "))
            {
                reply = reply.Substring(3).Trim();
            }

            int? i = DateTimeClass.GetHowManySeconds(reply);
            if (i == null)
            {
                return null;
            }
